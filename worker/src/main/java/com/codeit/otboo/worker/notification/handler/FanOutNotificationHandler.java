...
public class FanOutNotificationHandler implements NotificationRequestHandler {
    ...
    private NotificationHandlingResult weather(NotificationCreateMessage<JsonNode> message) {
        ...
        return save(message, receivers, "%d월 %d일 비 예보".formatted(rainAt.getMonthValue(), rainAt.getDayOfMonth()),
    ...
    private NotificationHandlingResult feed(NotificationCreateMessage<JsonNode> message) {
        ...
        return save(message, receivers, "새로운 피드가 등록되었습니다",
                "%s님이 새로운 피드를 등록했습니다.".formatted(source.authorName()),
                next, payload.feedId().toString());
    }

    private NotificationHandlingResult save(NotificationCreateMessage<JsonNode> message, List<UUID> receivers,
            String title, String content, Object nextPayload, String key) {
        var saved = saveService.savePage(message.type(), message.deduplicationKey(), receivers,
                title, content, NotificationLevel.INFO);
        var next = nextPayload == null ? null : new NotificationCreateMessage<>(message.eventId(),
                message.schemaVersion(), message.type(), message.occurredAt(),
                message.deduplicationKey(), nextPayload);
    ...
    private <T> T payload(NotificationCreateMessage<JsonNode> message, Class<T> type) {
        try {
            ...
            if (payload == null) throw new NotificationException(ErrorCode.INVALID_INPUT_VALUE);
            return payload;
